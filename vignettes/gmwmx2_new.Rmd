...
\]
where \(\boldsymbol{\varepsilon} \sim F\!\left(0, \sigma(\boldsymbol{\gamma})\right)\)
denotes a dependent‑error process with distribution family \(F\) and scale
structure \(\sigma(\boldsymbol{\gamma})\).

We simulate a time series as

`y = X %*% beta + epsilon`,

where:

1. `X` is a user-defined design matrix.
2. `beta` are the true regression coefficients.
3. `epsilon` is a stochastic process generated from a composite noise model (e.g., white
noise + AR(1), white noise + power-law, white noise + flicker).
