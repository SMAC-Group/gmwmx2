...
# Model and estimator
This vignette builds on the general framework described in the vignette
"Estimate linear models with dependent errors and missing observations." We do
not repeat the full model, missingness mechanism, or estimation details here.
Please refer to that vignette for the formal setup, and use this one as a
...
```{r}
fit1 <- gmwmx2_new(station_data, n_seasonal = 2, component = "N", model = wn())
fit1
```
...
```{r}
fit2 <- gmwmx2_new(station_data, n_seasonal = 2, component = "N", model = wn()+ ar1())
fit2
```
...
```{r}
fit3 = gmwmx2_new(station_data, n_seasonal = 2, component = "N", model = wn() + pl())
fit3
plot(fit3)
```

```{r}
fit4 = gmwmx2_new(station_data, n_seasonal = 2, component = "N", model = wn() + flicker())
fit4
plot(fit4)
```

```{r}
fit5 = gmwmx2_new(station_data, n_seasonal = 2, component = "N", model = wn() +  matern())
fit5
plot(fit5)
```
