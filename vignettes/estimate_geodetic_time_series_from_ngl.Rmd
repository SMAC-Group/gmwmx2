...
```{r}
fit2 <- gmwmx2_new(station_data, n_seasonal = 2, component = "N", model = wn()+ pl())
fit2
```
...
```{r}
fit3 = gmwmx2_new(station_data, n_seasonal = 2, component = "N", model = wn() + flicker())
fit3
```
...
```
```{r}
fit4 = gmwmx2_new(station_data, n_seasonal = 2, component = "N", model = wn() + ar1())
fit4
```
