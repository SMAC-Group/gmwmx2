...
with dependent errors and missing observations. We work with the model
\[
  \mathbf{Y} = \mathbf{X}\boldsymbol{\beta} + \boldsymbol{\varepsilon},
\]
where the error process \(\boldsymbol{\varepsilon}\) follows a composite stochastic model (e.g., white noise + AR(1)). Missingness is modeled by a
binary process \(\mathbf{Z} \in \{0,1\}^n\) with \(Z_t = 1\) indicating
observation, and \(Z_t = 0\) a msising observation. The observed series is
\[
