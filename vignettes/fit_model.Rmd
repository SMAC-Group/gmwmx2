...
# Model and estimator
The `gmwmx2` `R` package allows to estimate linear model with dependent errors described by composite stochastic model in presence of missing data.
...
We then estimate the parameters $\boldsymbol{\gamma}$ using a Generalized method of Wavelet Moments approach [@guerrier2013wavelet].
...
```{r}
fit1 <- gmwmx2_new(station_data, n_seasonal = 2, component = "N", stochastic_model = wn()+ pl())
fit1
```
...
```
```{r}
fit2 <- gmwmx2_new(station_data, n_seasonal = 2, component = "N", stochastic_model = wn()+ flicker())
fit2
```
...
```{r}
fit3 = gmwmx2_new(station_data, n_seasonal = 2, component = "N", stochastic_model = wn() + ar1())
fit3
plot(fit3)
```
