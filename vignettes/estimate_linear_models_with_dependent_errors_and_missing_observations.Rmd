...
Missingness is modeled by a binary process \(\mathbf{Z} \in \{0,1\}^n\) with \(Z_i = 1\) indicating
observation, and \(Z_i = 0\) a missing observation with $i \in \{1,\ldots, n\}$. The missingness process is independent of $\mathbf{Y}$ and respect the following definition:
\begin{equation}
  Z_i =
\begin{cases}
...
\begin{equation}
\mu(\hat{\boldsymbol{\vartheta}})^{-2}(\mathbf{X}^{T}\mathbf{X})^{-1} \mathbf{X}^{T} \boldsymbol{\Sigma}(\hat{\boldsymbol{\gamma}}, \hat{\boldsymbol{\vartheta}})\mathbf{X} (\mathbf{X}^{T}\mathbf{X})^{-1}$
\end{equation}
