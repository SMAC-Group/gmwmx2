...
You can set a random seed before calling `generate()` or directly specify the seed to make the output
reproducible. Using the same seed and model parameters
produces the exact same time series.

```{r}
set.seed(1234)
mod1 <- wn(sigma2 = 1)
series_a <- generate(model_wn_repro, n = 100)
series_b <- generate(model_wn_repro, n = 100, seed=1234)
series_c = generate(model_wn_repro, n = 100, seed=4321)
