...
composite model parametrized by \(\boldsymbol{\gamma} \in \boldsymbol{\Gamma}\).

Given a parametric model with parameters \(\boldsymbol{\gamma}\), the GMWM
estimator minimizes the weighted wavelet-variance discrepancy:
\[
  \hat{\boldsymbol{\gamma}} =
  \arg\min_{\boldsymbol{\gamma} \in \boldsymbol{\Gamma}}
  \left(\hat{\boldsymbol{\nu}} - \boldsymbol{\nu}(\boldsymbol{\gamma})\right)^\top
  \boldsymbol{\Omega}
